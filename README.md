...
第一版可用原型（Electron），核心功能已跑通，正在自用验证阶段。

已完成：

- 桌宠常驻窗（灵狐形象，可拖拽、单击开面板、状态随久坐/完成变化）。
- 「等 AI 回复间隙」智能触发 + 久坐/喝水兜底提醒，零系统权限。
- 23 个办公室微运动动作库（带步骤 + 注意事项）。
- 首次问卷引导 → 用户画像 → 加权推荐动作。
- 今日数据 + 设置面板；打包成独立 App（DMG / zip）。

进行中 / 计划：行为自适应推荐、App 排除名单（开会不弹）、多姿势形象。
